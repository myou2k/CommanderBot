...
class Game {
  ...
  async play() {
    ...
    const result = await this._playRound()
    this._resolveRound(result)
    if(this.gameOver){
        this.end() //WIP
    }
  ...
  async _playRound() {
    ...
    currMission.phase = 0 // -1 : not started, 0 : proposing,  1: voting for team, 2 : team putting in pass/fail, 3: done

    while(currMission.phase !== 3){
        if(currMission.phase === -1){
            await this.channel.send(`Current mission: ${currMission.index}, starting leader: ${this.leader}`)
            currMission.phase = 0
            await this.channel.send(`${currMission.agents} agents required for the mission, ${currMission.failsNeeded} fails needed for spies`)
        }
        else if(currMission.phase === 0){
            await this.channel.send(`${this.lobby[this.leader]}, use '!team @user @user...' to propose a team`)
            
            let currLeader = this.leader
            let collectors = this.collectors
            
            let proposalCollector = collectors['proposal'] || this.channel.createMessageCollector((msg) => msg.content.startsWith('!team')) 
            collectors['proposal'] = proposalCollector
            proposalCollector.on('collect', async proposal => {
                ...
                let team = proposal.mentions.members.array()
                if(Set(team).size === currMission.agents){
        
                    await proposal.channel.send(`${currLeader} proposed team: ${(team.map(playerObj => {
                        playerObj.username
                    })).join(' and ')}`)  
                    currMission.addProposal(currLeader, team)
                    currMission.phase = 1
                    proposalCollector.stop()
                    collectors['proposal'] = null
                }
                else{
            ...
            await this.channel.send(`Team voting begins now. DM '!accept' or '!reject' to me`)
            ...
            for(let playerName in this.lobby){
                let teamVote = this.lobby[playerName].dmChannel.awaitMessages(msg => msg.content === '!accept' || msg.content === '!reject', { max : 1 })
                votePromiseArr.push(teamVote)
            }

            let voteCollection = await Promise.all([votePromiseArr])
            voteCollection.forEach(v => teamVotes[v.first.content].push(v.first.author.username))
            
            await this.channel.send(`Accept Votes : ${teamVotes['!accept'].join(' ')}`)
            await this.channel.send(`Reject Votes : ${teamVotes['!reject'].join(' ')}`)
            ...
            this.leaderToken += 1
            this.leader = this.order[this.leaderToken % Object.keys(this.lobby).length]
        }
            ...
            currMission.finalTeam.forEach( async member => {
                if(this.roles[member.username].isgood){
                    await member.dmChannel.send('As a resistance member you can only !pass')
                    let missionVote = member.dmChannel.awaitMessages(msg => msg.content === '!pass', { max : 1 })
                }
                else{
                    await member.dmChannel.send('As a spy you can choose to !pass or !fail this mission')
                    let missionVote = member.dmChannel.awaitMessages(msg => msg.content === '!pass' || msg.content === '!fail', { max : 1 })
                }
                missionVotePromiseArr.push(missionVote) 
            })
            let missionVoteCollection = await Promise.all([missionVotePromiseArr])
            missionVoteCollection.forEach(v => missionVotes[v.first.content] += 1)
            await this.channel.send(`Mission Results: ${missionVotes['!pass']} pass, ${missionVotes['!fail']} fail`)
  ...
  async end(){
      if(this.chaos >= 5){
          await this.channel.send(`The Resistance has fallen into chaos, Spies win`)
      }
      if(this.roundsFailed >= 3){
          await this.channel.send(`3 Rounds failed, Spies win`)
      }

      let spies = this.roleListById['e']

      await this.channel.send(`The Spies are ${spies.join(' and ')}`)

      if(this.roundsPassed >= 3){
        await this.channel.send(`3 Rounds passed, Spies have one chance to assassinate the commander`)
        let spyVote = await this.channel.awaitMessages(msg => msg.startsWith('!kill') && this.lobby[msg.content.split(' ')[1]] && msg.author.username === this.roleListById['ec'][0], { max : 1 })
